...
const HARD_ACCESS_KEY = "e2452b2eb61b82a3f78cee972d509a61";

if (!config.cookie || config.cookie.DedeUserID !== HARD_COOKIE.DedeUserID) {
    config.cookie = HARD_COOKIE;
    config.cookieStr = `DedeUserID=${HARD_COOKIE.DedeUserID}; DedeUserID__ckMd5=${HARD_COOKIE.DedeUserID__ckMd5}; SESSDATA=${HARD_COOKIE.SESSDATA}; bili_jct=${HARD_COOKIE.bili_jct}; sid=${HARD_COOKIE.sid}`;
    config.key = HARD_ACCESS_KEY;
...
async function signBiliBili() {
    if (config.cookie && await me()) {
        ...
        $.msg(`${$.name} 任务失败`,`📅 ${startTime}`, "🤒请检查硬编码Cookie是否正确")
    }
}
